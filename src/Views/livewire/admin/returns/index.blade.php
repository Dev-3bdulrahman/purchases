<div class="p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('purchases::purchases.returns') }}</h2>
            <p class="text-gray-500 text-sm mt-1">{{ __('purchases::purchases.manage_returns') }}</p>
        </div>
        <button wire:click="openCreateModal"
            ...
            <span>{{ __('purchases::purchases.add_return') }}</span>
        </button>
    </div>
            ...
            <table class="w-full text-right border-collapse">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('purchases::purchases.return_number') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('purchases::purchases.supplier') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('purchases::purchases.return_date') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('purchases::purchases.invoice') }}</th>
                    ...
                    @forelse($returns as $ret)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                                {{ $ret->supplier ? $ret->supplier->name : '-' }}
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $statusClasses[$ret->status] ?? $statusClasses['pending'] }}">
                                    {{ __('purchases::purchases.' . $ret->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    <button wire:click="openEditModal({{ $ret->id }})" title="{{ __('purchases::purchases.edit_return') }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                <span>{{ __('purchases::purchases.no_returns') }}</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-6 border-b border-gray-50 dark:border-gray-800 pb-4">
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                                {{ $returnId ? __('purchases::purchases.edit_return') : __('purchases::purchases.add_return') }}
                            </h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('purchases::purchases.invoice') }}</label>
                                <select wire:model="supplier_invoice_id" class="w-full py-2 px-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white">
                                    <option value="">{{ __('purchases::purchases.select_invoice') }}</option>
                                    @foreach($invoices as $i)
                                        <option value="{{ $i->id }}">{{ $i->invoice_number }}</option>
                                    @endforeach
                                </select>
                                @error('supplier_invoice_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('purchases::purchases.return_number') }} *</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('purchases::purchases.return_date') }} *</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('purchases::purchases.reason') }}</label>
                                <textarea wire:model="reason" rows="4" class="w-full py-2 px-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white"></textarea>
