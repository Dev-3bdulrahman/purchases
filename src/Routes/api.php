...
use Dev3bdulrahman\Purchases\Http\Controllers\Api\GoodsReceiptNoteApiController;
use Dev3bdulrahman\Purchases\Http\Controllers\Api\ThreeWayMatchApiController;
...
Route::prefix('api/v1/purchases')->middleware(['auth:sanctum', 'throttle:60,1', 'api.tenant'])->group(function () {
    ...
    // Goods Receipt Notes
    Route::get('goods-receipt', [GoodsReceiptNoteApiController::class, 'index'])->middleware('can:purchases.goods-receipt.view')->name('api.v1.purchases.goods-receipt.index');
    Route::post('goods-receipt', [GoodsReceiptNoteApiController::class, 'store'])->middleware('can:purchases.goods-receipt.create')->name('api.v1.purchases.goods-receipt.store');
    Route::get('goods-receipt/{goodsReceiptNote}', [GoodsReceiptNoteApiController::class, 'show'])->middleware('can:purchases.goods-receipt.view')->name('api.v1.purchases.goods-receipt.show');
    Route::put('goods-receipt/{goodsReceiptNote}', [GoodsReceiptNoteApiController::class, 'update'])->middleware('can:purchases.goods-receipt.update')->name('api.v1.purchases.goods-receipt.update');
    Route::delete('goods-receipt/{goodsReceiptNote}', [GoodsReceiptNoteApiController::class, 'destroy'])->middleware('can:purchases.goods-receipt.delete')->name('api.v1.purchases.goods-receipt.destroy');
    Route::post('goods-receipt/{goodsReceiptNote}/complete', [GoodsReceiptNoteApiController::class, 'complete'])->middleware('can:purchases.goods-receipt.complete')->name('api.v1.purchases.goods-receipt.complete');
    Route::post('goods-receipt/{goodsReceiptNote}/receive-items', [GoodsReceiptNoteApiController::class, 'receiveItems'])->middleware('can:purchases.goods-receipt.receive')->name('api.v1.purchases.goods-receipt.receive-items');

    // Three-Way Matching
    Route::post('matching/match', [ThreeWayMatchApiController::class, 'match'])->middleware('can:purchases.matching.view')->name('api.v1.purchases.matching.match');
    Route::get('matching/status', [ThreeWayMatchApiController::class, 'status'])->middleware('can:purchases.matching.view')->name('api.v1.purchases.matching.status');
    Route::get('matching/report', [ThreeWayMatchApiController::class, 'report'])->middleware('can:purchases.matching.view')->name('api.v1.purchases.matching.report');
    Route::post('matching/resolve', [ThreeWayMatchApiController::class, 'resolve'])->middleware('can:purchases.matching.resolve')->name('api.v1.purchases.matching.resolve');
});
